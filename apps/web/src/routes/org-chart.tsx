...
function OrgChartReveal({ batchId, initialChart }: { batchId: string; initialChart: OrgChart }) {
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-16">
      <div className="mb-10 space-y-2 duration-ceremony-slow ease-ceremony">
        <p className="font-mono text-xs uppercase tracking-[0.2em] text-text-muted">Your company</p>
        <h1 className="font-display text-3xl font-semibold">
          Your idea needs a company of {nonHumanTeams.length} roles running {chart.tasks.length} tasks. Here it is.
        </h1>
      </div>
          ...
          {chart.teams.map((team, teamIndex) => (
            ...
            >
              <h2 className="font-display text-sm font-semibold uppercase tracking-wide text-text-secondary">
                {team.roleTitle}
                {team.isHuman && <span className="ml-2 text-text-muted">(you)</span>}
              </h2>
