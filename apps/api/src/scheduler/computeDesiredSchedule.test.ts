...
test("schedules a cadence-triggered task with its owning agent, jobSchedulerId scoped by tenant/agent/task", () => {
  ...
  const { desired } = computeDesiredSchedule("tenant-1", "scale", chart);
...
test("clamps a task's cadence to the tenant's tier floor and surfaces the reason", () => {
  const chart = makeChart([makeAgent()], [makeTask({ cadence: "15min", triggerType: "cadence" })]);
  const { desired, clampNotes } = computeDesiredSchedule("tenant-1", "solo", chart);

  assert.equal(desired[0].cadence, "daily");
  assert.equal(clampNotes.length, 1);
  assert.equal(clampNotes[0].taskId, "task-1");
  assert.match(clampNotes[0].reason, /Runs daily on Solo/);
});

test("no clamp note when the declared cadence is already within the tier floor", () => {
  const chart = makeChart([makeAgent()], [makeTask({ cadence: "weekly" })]);
  const { clampNotes } = computeDesiredSchedule("tenant-1", "solo", chart);
