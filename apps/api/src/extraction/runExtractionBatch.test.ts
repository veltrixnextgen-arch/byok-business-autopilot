...
test("a successful batch reserves, settles with the real cost, and persists it", async () => {
...
// persisted batch record (batchStore.fail's error argument) for operators,
// never in what's returned to the caller.
test("a SKIP verdict returns a plain-language reason to the caller, not the internal ceiling-jargon string", async () => {
  ...
  // The detailed, technical reason is still recorded for operators, in the
  // persisted batch record.
  assert.match(batchStore.failedWith[0] ?? "", /ceiling/i);
});
...
// GET /extraction/batches/latest). This is a regression test, not proof
// the Anthropic SDK itself never echoes a key in an error (out of this
// repo's control) — it guards the realistic failure mode: something in
// OUR error handling accidentally interpolating deps.apiKey into a
// message that then gets persisted.
test("the platform API key never appears in a persisted error, even on failure", async () => {
