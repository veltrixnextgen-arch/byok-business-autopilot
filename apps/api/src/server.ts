...
export function readServerConfigFromEnv(env: NodeJS.ProcessEnv = process.env): ServerConfig {
  ...
  return { port, databaseUrl, authSecret, authBaseUrl };
}
...
export function startServer(config: ServerConfig, trustCore: TrustCoreDeps) {
  ...
  const auth = createAuth({ db, baseURL: config.authBaseUrl, secret: config.authSecret });
  const app = createApp({ pool, auth, trustCore });
