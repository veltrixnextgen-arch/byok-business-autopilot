...
- **Still open (PR B)**: the hero's two-column layout and its animated illustration (a human figure inside a constellation/network diagram whose node positions appear to run a live force-directed simulation — reproducing that exact behavior would need a physics/charting dependency, which is out of bounds; PR B instead hand-authors an SVG animated with CSS transform/opacity, "alive" via varied per-node orbit/breathe/drift durations, not a literal physics port). Also open: per-step illustration evolution in the scroll sequence, the nav's logo mark, and the Spending Walls section's card restructure (one card with a real progress bar, not four equal-weight stat cards).
