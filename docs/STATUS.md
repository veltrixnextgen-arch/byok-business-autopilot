...
2. **Effect-dispatch is no longer draft-only for everything (ADR-043 partially superseded) — confirmed live, real inbox, one narrow task type.** `support.digest.weekly-summary` (SaaS template) proposes a real "send" effect; `ResendEffectExecutor` (`packages/approval-queue`) sends it via the tenant's own connected Resend key, only after a human APPROVE/MODIFY — earned autonomy is structurally unable to bypass an effect-bearing action regardless of task type (`queue.ts`'s `submitProposedAction`). Backfilled into Acme deliberately, with explicit real-tenant authorization, specifically to make this observation possible (PR #218 shipped the mechanism; the backfill itself was a one-off script, not a merged migration — this task type doesn't exist on any other real tenant's chart yet). **The founder connected a real Resend key, approved the proposed send through the real Approvals UI, and confirmed the email landed — 2026-09-03.** Three more real gaps found and fixed in the process, all merged: `AgentsScreen.tsx` crashed for every one of the 5 real tenants on a second half of the same JSONB-drift class PR #215 fixed (`objective`/`reportingStructure`, PR #219); `/org-chart` showed a stale, never-claimed draft instead of the real company whenever both existed on one account (PR #220); the API/UI never actually surfaced whether a dispatched effect succeeded or failed — `dispatched: true` only meant "the executor ran" (PR #221). Still a proof of the mechanism for one task type, not a general "agents can send anything" system — every other task type stays draft-only.
...
Full detail in `docs/DECISIONS.md`, ADR-039 through ADR-059, plus PR #210/#212/#213/#215/#217/#218/#219/#220/#221.
...
| ADR-043 | Effect-dispatch stays draft-only for all of MVP-1/Phase 2 — a decision, not an open TBD. **Partially superseded by PR #218 below**, for one task type only, confirmed live with a real send. | Documented as a closed question with the three options actually considered |
...
| **PR #218** | Week 1's narrow real-effect-dispatch scope: one task type (`support.digest.weekly-summary`, SaaS template) proposes a real "send" effect; `ResendEffectExecutor` sends it via the tenant's own connected Resend key, to the tenant's own owner/admin emails, only after a human APPROVE/MODIFY. Closed a real safety gap found while building it: `ApprovalQueue.submitProposedAction`'s earned-autonomy bypass could dispatch an effect with zero human review — now structurally impossible for any effect-bearing action, not just this one | New/updated unit tests across `packages/approval-queue` (46 tests), `apps/router` (60), `apps/api` (258) — full suite green. **Confirmed live 2026-09-03**: backfilled into Acme, Resend connected, approved through the real UI, real email confirmed received |
| **PR #219** | Extended `normalizeOrgChart` to backfill `objective`/`reportingStructure` too, not just `budget`/`riskTier` — found live when `AgentsScreen.tsx`'s unguarded `agent.reportingStructure.teamRoleTitle` crashed the whole `/agents` screen for every one of the 5 real tenants. Same contract addition (2026-08-28/29) PR #215 only half-fixed | 3 new unit tests; no separate data backfill needed, the read-time fix repairs every read immediately |
| **PR #220** | `/org-chart` showed a stale, never-claimed pre-org batch instead of a user's real claimed company, whenever an account had both — `getLatestBatch` (pre-org) was tried before `getOrgChartForTenant` (claimed-org), so a genuinely successful stale read meant the real chart was never even checked. Swapped the priority | Rewrote the 2 tests that encoded the old, backwards order; added the exact regression case |
| **PR #221** | The API/UI never surfaced whether a dispatched effect actually succeeded — `POST /:id/resolve` returned only `{resolved, dispatched}`, and `dispatched: true` meant "the executor ran," not "it worked." `ResendEffectExecutor`'s own `{success:false, error}` was computed correctly and silently dropped | Route now returns `effectResult` and logs a failure loudly server-side; `ApprovalsScreen` shows a real success/failure banner |
...
2. Submit Google OAuth verification — code side and the privacy-policy blocker (PR #217) are both done; submission itself is the remaining step.
3. Week 2 per the current plan: Google Calendar real effect-dispatch once verification clears, then a real Hands connect flow for more task types beyond the one proven this session.
