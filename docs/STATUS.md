...
2. **In progress: systemic fix for JSONB schema drift** (this snapshot's new, unnumbered issue) — a migration-on-read step so the next `Agent`/`Task` contract field doesn't reproduce PR #213's failure mode on every pre-existing stored chart.
