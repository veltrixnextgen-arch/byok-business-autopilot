...
## Known state: three hero fidelity items pending exact-value tooling (2026-08-08)

The hero (`apps/web/src/components/landing/{Hero,HeroPanel}.tsx`) was fixed against the design reference (`company-generator.preview.emergentagent.com`) using screenshot-based visual measurement — the reference's actual content renders inside a cross-origin `app.emergent.sh` iframe, so `getComputedStyle()` cannot reach it from a same-origin script (confirmed via direct `curl` of the top-level HTML and a failed in-page evaluate). `chrome-devtools-mcp` was set up specifically to get around this (real DevTools access via CDP can reach cross-origin frames where page-context JS can't), but this environment does not appear to support hot-loading a newly-approved MCP server into an already-running session — confirmed twice, including after a full session restart, via direct tool-registry search for the package's actual tool names (`navigate_page`, `evaluate_script`, `take_snapshot`, etc.), none of which appeared even though `claude mcp list` reported the server "Connected" at the config level.

Two items were fixed this round using visual measurement (vertical rhythm — hero section padding tightened so the fold now fits eyebrow/headline/subhead/input/"See how it works"; node label density — the illustration's dense field cut from 10 labels to 6 and stopped duplicating "Operations"/"Marketing" against the team-ring's own labels). Three were deliberately left as documented deviations rather than guessed at with visual-only measurement a second time:

1. **Headline wrap** — reference wraps "Nobody handed you" onto one intact line (3 lines total); ours orphans "you" onto its own line (4 lines). Needs the reference's actual computed container max-width and font-size, not a guess.
2. **Panel frame** — reference's bordered panel and the figure's visibility inside it need the reference's actual border/background/opacity values, not an approximation.
3. **Scroll sequence proportions** — Step 1's illustration placement/sizing across all six steps needs the reference's actual computed layout, not a repeat of the same measurement method already found insufficient here.

**Next step, if picked back up:** get chrome-devtools-mcp (or equivalent CDP access) actually working in a live session before attempting these three again — re-attempting with the same screenshot-based method that already produced two rounds of close-but-wrong results isn't worth another cycle.
