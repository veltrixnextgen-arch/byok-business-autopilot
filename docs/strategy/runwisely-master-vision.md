...
Each agent carries: role, objective, responsibilities, tasks, required capabilities, tools, AI model, memory, permissions, budget, approval requirements, reporting structure, escalation rules. Never create unnecessary agents.
...
| Roles → agents with full metadata | **Partially built.** The real `Agent` type (`packages/agents/contracts/src/orgChart.ts`) carries id, name, title, teamId, taskIds, tier, brain (nullable), hands (tool names), autonomyDefault, complianceLocked, requiresProfessionalVerification — task-level cadence lives on `Task`, not `Agent`. **Not present as fields on either type: objective, budget, memory, escalation rules, explicit reporting structure** (only implicit via `teamId`/`Team`). §8's "remaining budget, projected spend" language describes company-level CostGate ceilings (§9's Spend Protocol row), not a per-agent field — no per-agent budget exists in code today. |
...
| Continuous OBSERVE→…→LEARN loop | **Partial.** Observe/think/plan/delegate/execute/verify exist. Learn does not — template deltas are now captured but nothing consumes them. |
...
7. Learning: aggregation and redaction over captured deltas, then human-reviewed template proposals.
