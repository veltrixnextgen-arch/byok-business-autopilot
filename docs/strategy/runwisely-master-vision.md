...
| Agent Spend Protocol | **Built at company level; per-day is now genuinely per-agent.** Per-tenant durable ceilings, fail-closed gate, pause-and-resume, real cost per sub-agent. The `"task-type-day"` ceiling level (`packages/cost-gate/src/durable/reservationStore.ts`, migration `0020`) resets daily with no cron job, and `CeilingConfig.perTaskTypePerDayUsd` (migration `0021`'s trust-core wiring) is now a real per-agent map — both trust cores read the tenant's own org chart and populate it from each agent's own `budget.perDayUsd`, falling back to the flat `DEFAULT_PER_AGENT_PER_DAY_USD` only for onboarding-time task types that have no agent to draw from. The values themselves are still tier-derived defaults, not informed per-agent numbers (see the roles-→-agents row above) — a T1 agent's cap differs from a T3 agent's, but two T1 agents share the same number. **Still missing:** any product surface to *set* a per-agent override (read-only visibility shipped in `AgentsScreen.tsx`; editing needs its own persistence, scoped out — see §10), and `perRoleUsd`/`perTaskTypeUsd` (the two non-day levels) remain wired to `{}` in both trust cores. |
