...
*Last reconciled 2026-09-04, against commit history through PR #213/#215/#217/#218/#219/#220/#221/#227/#232/#233 (all merged).*
...
| BYOK / BYOC | **Built for Brains.** Four providers validated, per-role keys, vault-encrypted. Hands are mostly OAuth-pending. **A real bug closed, PR #227:** a Hands tool that genuinely had a live OAuth connect path (Google Calendar) could still land in the OAuth-pending UI branch because customize.ts's `handsTool` output is free text with no enum constraint against the real registry (`HANDS_AUTH_METHOD`) — found on a real tenant's Scheduling agent, fixed at `normalizeOrgChart`'s read-time chokepoint so every affected chart self-corrects, not just new ones. **A second, deeper bug closed, PR #233:** even with the label fixed, the connect button led nowhere real — the OAuth callback's `redirectUri` pointed at the raw Railway domain instead of the proxied web origin the session cookie is scoped to, so every real connect attempt failed closed with `state_mismatch`. Found only by actually walking the live flow while recording the verification demo, not by inspecting config. Fixed in code; Google Cloud Console's registered redirect URI still needs the matching update before a real connection can complete. |
...
4. **Google OAuth verification submitted** — the shortest path from "we draft" to "we act," which is the category's own dividing line. **Still open** — this is a real action item (submit the verification request), not a build task. Preparing the demo video that submission needs found three real bugs blocking it (all fixed, PR #227/#232/#233): a Hands-tool label drift that made a working Calendar connect button show as "draft only"; a claimed org chart with no `onboardingBatch` that couldn't draft a Charter at all (now self-heals via a real `generateOnboardingBatch` call); and — found live, mid-recording, once the first two were cleared — the connect flow itself failing every attempt with `state_mismatch`, because the OAuth callback's `redirectUri` pointed at the raw Railway domain instead of the proxied web origin, so Better Auth's session cookie never reached it. Google Cloud Console's registered redirect URI still needs updating to match the fix (a console action, not code) before a real connect can complete end to end.
